...
CollegeStudent::CollegeStudent(float x, float y, GameMap* gamemap) {
    movement.x = 0;
    movement.y = 0;
    xPosition = x;
    yPosition = y;
...
void CollegeStudent::generateItem() {
...
void CollegeStudent::setMoving(bool move) {
    moving = move;
}

bool CollegeStudent::isMoving() {
    return moving;
}
...
void CollegeStudent::followEnemy() {
...
void CollegeStudent::randomMove() {
    srand(chrono::high_resolution_clock::now().time_since_epoch().count());
...
void CollegeStudent::Update(sf::RenderWindow* Window) {
...
void CollegeStudent::resetDistance() {
    sf::FloatRect bounds = getSprite()->getGlobalBounds();
    distanceX = 0;
    distanceY = 0;
...
void CollegeStudent::setup() {
    ...
    walkingAnimationRight.addFrame(sf::IntRect(128, 224, 32, 32));
    walkingAnimationRight.addFrame(sf::IntRect(128, 256, 32, 32));

    walkingAnimationLeft.setSpriteSheet(texture);
    walkingAnimationLeft.addFrame(sf::IntRect(128, 64, 32, 32));
    walkingAnimationLeft.addFrame(sf::IntRect(128, 224, 32, 32));
    walkingAnimationLeft.addFrame(sf::IntRect(128, 256, 32, 32));

    walkingAnimationUp.setSpriteSheet(texture);
    walkingAnimationUp.addFrame(sf::IntRect(128, 32, 32, 32));
    walkingAnimationUp.addFrame(sf::IntRect(128, 96, 32, 32));
    walkingAnimationUp.addFrame(sf::IntRect(128, 128, 32, 32));

    walkingAnimationDown.setSpriteSheet(texture);
    walkingAnimationDown.addFrame(sf::IntRect(128, 0, 32, 32));
    walkingAnimationDown.addFrame(sf::IntRect(128, 160, 32, 32));
    walkingAnimationDown.addFrame(sf::IntRect(128, 192, 32, 32));
